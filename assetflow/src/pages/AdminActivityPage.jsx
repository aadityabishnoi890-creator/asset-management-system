...
export function AdminActivityPage() {
  ...
  if (loading) {
    return <div className="py-10 text-sm text-slate-500">Loading activity…</div>
  }
  ...
  return (
    <div className="space-y-5">
      ...
      {/* Header */}
      <div>
        <h1 className="text-xl font-semibold text-slate-900">All Activity</h1>
        <p className="text-sm text-slate-500 mt-0.5">
          {bookings.length} total booking{bookings.length !== 1 ? 's' : ''} across all users
        </p>
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        ...
        <div className="relative flex-1 max-w-sm">
          ...
          <input
            ...
            placeholder="Search by user, asset, purpose…"
            value={search}
      ...
      {filtered.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-20 text-center">
          <History className="text-slate-300 mb-3" size={40} />
              ...
              return (
                <div key={b.id} className="bg-white rounded-xl border border-slate-200 p-4">
                  <div className="flex items-start justify-between gap-3 mb-2">
                    <div>
                      <p className="text-sm font-medium text-slate-900">{b.asset?.name}</p>
                      <p className="text-xs text-slate-400">{b.user?.name} • {b.user?.email}</p>
                    </div>
                    <span className={`inline-flex items-center px-2 py-0.5 rounded-md text-xs font-medium flex-shrink-0 ${STATUS_STYLES[b.status] || 'bg-slate-100 text-slate-600'}`}>
                      {b.status}
                    </span>
                  </div>
                  <div className="flex items-center gap-3 text-xs text-slate-500">
                    <span>Qty: {b.quantity}</span>
                    <span>{b.startDate} → {b.dueDate}</span>
                    {overdue && <span className="text-red-600 font-medium">Overdue</span>}
                  </div>
                  {b.purpose && <p className="text-xs text-slate-400 mt-1 truncate">{b.purpose}</p>}
                </div>
              )
            })}
          </div>
